...
- Treat [03_SDK_HANDOFF.md](notes/03_SDK_HANDOFF.md) as documentation of the **remote Teardrop API contract** this SDK wraps. It was written for a TypeScript SDK build (camelCase names) — map to this repo's snake_case methods/models and verify against live client code before citing behavior as current; it also records dated "additive API delta" notes showing what changed server-side over time.
